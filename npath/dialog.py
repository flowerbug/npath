...
#        print ("Use Current Parameters")
        if (cfg.show_board != 2):
            DialogWin (
                "         Npath Configuration File Doesn't Exist         ",
...
#        print ("Creating : ", str(cfg.config_path))
        ...
        DialogWin (
            "          Npath No Configuration Directory Exists            ",
...
class MyConfigWindow(Gtk.ApplicationWindow):
    ...
    def __init__(self, app):
        Gtk.Window.__init__(self, title="Npath Configuration", application=app)
...
#        print ("Restore Defaults")
        ...
        if (cfg.show_board != 2):
            DialogWin (
                "            Npath Configuration Parameters Reset           ",
...
class TextViewWindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        Gtk.Window.__init__(self, title="About Npath")
...
class HelpViewWindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        Gtk.Window.__init__(self, title="Npath Help")
    ...
    def create_textview(self):
        ...
        self.textbuffer = self.textview.get_buffer()
        self.textbuffer.set_text(
            "\n  Npath is a puzzle game.  When Npath is first started you will be shown on the left a blank grid called the Guess grid and on the right is a selection of mirrors.  Under each mirror is the number that need to be placed on the Guess grid to solve the puzzle.  You have to use all of the mirrors.\n"
            + "\n"
...
#    print ("Load_NPATHSave_Version_1 -> new variables NC NR NB NWC", cfg.new_game_cols, cfg.new_game_rows, cfg.new_board, cfg.new_widget_counts)
...
#    print ("Keep track of current directory : ", cfg.saved_dir)
    ...
    if (cfg.data_path.exists() != True):
        DialogWin (
            "            Npath Save Game Directory Missing              ",
    ...
    if (len(os.listdir(path=str(cfg.data_path))) == 0):
        DialogWin (
            "            Npath Save Game Directory Empty                ",
...
#    print ("Open File selected : ", cfg.this_fn_to_open)
    if (cfg.this_fn_to_open.endswith(".gfp") == True):
        try:
            ...
            DialogWin (
                "            Npath Loading A Previously Saved Game Failed   ",
...
#        print ("No save directory exists.  Creating : ", str(cfg.data_path))
        ...
        DialogWin (
            "            Npath No Save Game Directory Exists            ",
